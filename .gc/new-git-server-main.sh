...
gitcid_new_git_server_main() {
  ...
  for j in $@; do
    ...
    if [ $gc_new_git_server_setup_sudo -eq 0 ]; then
      ...
      echo ""
      { bash -c "{ ssh -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=5 -o ConnectionAttempts=2 -tt ${gc_ssh_username}@${gc_ssh_host} 'alias sudo=\"sudo -n\"; echo \"\"; echo \"-----\"; echo \"  hostname: $(hostname)\"; echo \"  user: $USER\"; echo \"-----\"; bash <(curl -sL https://tinyurl.com/git-server-init); exit 21;'; loop_res2=$?; }"; loop_res2=$?; } & tasks+=( $! )
