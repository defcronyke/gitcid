...
gitcid_new_git_server() {
  ...
  for i in $@; do
    {  trap 'for i in $tasks; do kill $i; done; gitcid_new_git_server_usage; return 255' INT; ssh -tt $i "/bin/bash -c \"trap 'for i in $tasks; do kill $i; done; gitcid_new_git_server_usage; return 255' INT; echo \"\"; echo \"-----\"; echo \"hostname: $(hostname)\"; echo \"-----\"; curl -sL https://tinyurl.com/git-server-init | bash && exit $?\ || exit $?"" && exit $? || exit $?; } &
    tasks+=($!)
  done
